...
function CartPage({
  ...
  submitOrder: () => void;
  submitted: boolean;
  lastOrderDetails: { name: string; phone: string } | null;
}) {
  const navigate = useNavigate();
  if (submitted)
    return (
      ...
      <div className="order-form">
        {lastOrderDetails && (
          <div className="last-order-details">
            <p>Last order details:</p>
            <button 
              className="use-last-details"
              onClick={() => {
                setName(lastOrderDetails.name);
                setPhone(lastOrderDetails.phone);
              }}
            >
        ...
        <input 
          ...
          className={lastOrderDetails?.name === name ? 'prefilled' : ''}
        />
        <input 
          placeholder="Phone" 
          value={phone} 
          onChange={e => setPhone(e.target.value)}
          className={lastOrderDetails?.phone === phone ? 'prefilled' : ''}
        />
        <button 
          onClick={submitOrder} 
          disabled={!cart.length || !name || !phone}
        >
          Place Order
        </button>
      </div>
...
export default function App() {
  ...
  const [lastOrderDetails, setLastOrderDetails] = useState<{ name: string; phone: string } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/menu`)
      ...
      .then(data => {
        console.log("DEBUG: Menu received from backend:", data);
        setMenu(data);
        setError(false);
      })
      .catch(err => {
  ...
  const submitOrder = () => {
    // Save user details to localStorage after successful order
    ...
    fetch(`${API_URL}/order`, {
      ...
      setName("");
      setPhone("");
      setLastOrderDetails({ name, phone });
    });
  };
        ...
        <Route
          ...
          element={
            loading ? (
              ...
              <div className="centered">
                <h2>Server is starting up...</h2>
                <p>Please wait a moment while we connect to the backend server.</p>
                <p>This may take up to 30 seconds if the server was idle.</p>
